...
(deftest save-load-database-test
  (let [resource (test/create-resource "person")
    ...
    (save-resource test/database.cassandra resource)
    (let [stored (load-resource test/database.cassandra "person" id)]
...
(deftest load-non-existing-test
  (is (nil? (load-resource test/database.cassandra "person" "nothing"))))
